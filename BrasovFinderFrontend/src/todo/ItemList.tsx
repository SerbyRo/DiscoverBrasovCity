...
const ItemList: React.FC<RouteComponentProps> = ({ history }) => {
  ...
  useEffect(()=>{
    ...
    if(searchText && items){
      setMoviesShow(items.filter((item) => item.name.startsWith(searchText)));
    }
  },[searchText]);
  ...
  useEffect(()=>{
    if(filter && items){
      if(filter === "<=10 lei") {
        setMoviesShow(items.filter((item) => item.price <= 10) );
      }
      else if(filter === ">10 lei") {
        setMoviesShow(items.filter((item) => item.price > 10) );
      }
      else if(filter === "any number of lei") {
        setMoviesShow(items);
      }
    }
  }, [filter]);
    ...
    <IonPage>
      <IonHeader>
        ...
        <IonToolbar>
          ...
          <IonTitle className="page-title">Places Management</IonTitle>
        ...
        <IonItem>
          <IonLabel>Filter places by name</IonLabel>
      ...
      <IonContent>
        ...
        {items && moviesShow.map((item: ItemProps) => {
          return(
              <IonList>
                <Item key={item._id} _id={item._id} name={item.name} boooked_date={item.boooked_date}
                      visited={item.visited} price={item.price} lng={item.lng} lat={item.lat} photoPath={item.photoPath}
